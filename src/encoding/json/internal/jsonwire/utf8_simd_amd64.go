...
// consumeUTF8Prefix skips complete, valid UTF-8 runes that need no JSON,
// HTML, or JavaScript escaping. Errors and the final partial block are left
// to the scalar caller. ASCII starts are left to the cheaper ASCII scanner.
func consumeUTF8Prefix(b []byte) int {
	if len(b) >= 32 && b[0] >= 0x80 && archsimd.X86.AVX2() {
		return consumeUTF8PrefixAVX2(b)
	}
	return 0
}

func consumeUTF8PrefixAVX2(b []byte) (n int) {
	c0 := archsimd.BroadcastUint8x32(0xc0)
	e0 := archsimd.BroadcastUint8x32(0xe0)
	f0 := archsimd.BroadcastUint8x32(0xf0)
	fe := archsimd.BroadcastUint8x32(0xfe)
	x80 := archsimd.BroadcastUint8x32(0x80)
	...
	for len(b)-n >= 32 {
		...
		stop := v.And(e0).Equal(zero).
			Or(v.Equal(archsimd.BroadcastUint8x32('"'))).
			Or(v.Equal(archsimd.BroadcastUint8x32('\\'))).
			Or(v.Equal(archsimd.BroadcastUint8x32('&'))).
			Or(v.Or(archsimd.BroadcastUint8x32(2)).Equal(archsimd.BroadcastUint8x32('>'))).ToBits()
		e2 := v.Equal(archsimd.BroadcastUint8x32(0xe2)).ToBits()
		b80 := v.Equal(x80).ToBits()
		a8 := v.And(fe).Equal(archsimd.BroadcastUint8x32(0xa8)).ToBits()
		stop |= ((e2 << 2) & (b80 << 1) & a8) >> 2
		end := bits.TrailingZeros32(stop)
		mask := uint64(1)<<end - 1
		lead2 &= mask
		lead3 &= mask
		lead4 &= mask
		...
		// continuation bytes. A 64-bit mask retains carries beyond byte 31.
		must := (lead2|lead3|lead4)<<1 | (lead3|lead4)<<2 | lead4<<3
		invalid := uint64(v.And(fe).Equal(c0).
			Or(v.Greater(archsimd.BroadcastUint8x32(0xf4))).ToBits())
		lowCont := uint64(v.And(e0).Equal(x80).ToBits()) // 80..9f
		high80 := uint64(high.Equal(x80).ToBits())       // 80..8f
		e0s := uint64(v.Equal(e0).ToBits())
		eds := uint64(v.Equal(archsimd.BroadcastUint8x32(0xed)).ToBits())
		f0s := uint64(v.Equal(f0).ToBits())
		f4s := uint64(v.Equal(archsimd.BroadcastUint8x32(0xf4)).ToBits())
